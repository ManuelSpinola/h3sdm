...
if (requireNamespace("waffle", quietly = TRUE)) library(waffle)
...
```{r}
#| echo: false
ggplot(counts_current, aes(fill = habitat_cat, values = n)) +
  geom_waffle(n_rows = 40, size = 0.1, color = "white", flip = TRUE) +
  ...
  facet_wrap(~habitat_cat, 
             nrow = 1,
             strip.position = "bottom") +
  theme(
  strip.background = element_rect(fill = "gray90", color = "black"),
  strip.text = element_text(color = "black", face = "bold",
                            margin = margin(t = 2, r = 4, b = 2, l = 4))  # top, right, bottom, left
)
```
...
```{r}
#| echo: false
ggplot(counts_future, aes(fill = habitat_cat, values = n)) +
  geom_waffle(n_rows = 40, size = 0.1, color = "white", flip = TRUE) +
  scale_fill_manual(values = c(
    "Very low" = "#d73027",
    "Low" = "#fc8d59",
...
```{r}
#| echo: false
ggplot(counts_all, aes(fill = habitat_cat, values = n)) +
  geom_waffle(n_rows = 60, size = 0.1, color = "white", flip = TRUE) +
