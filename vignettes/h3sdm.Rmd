---
title: "h3sdm workflow for species distribution modeling using a single model"
...
Here we obtain presence-absence data for the species of interest (Silverstoneia flotator). We are using the `h3sdm_pa` function to get both presence and pseudo-absence records. The limit is set to 10,000 records and 300 pseudo-absences are generated.  There are various methods for generating pseudo-absences; here we use random sampling. The limit of 10,000 is only to ensure that all presence records are retrieved. Since there are approximately 100 positive hexagons at resolution 7, we request three times that number (i.e., 300) of pseudo-absences.
...
## 11. Model Interpretation: Feature Importance & Partial Dependence
...
This workflow can be adapted for multiple or more complex models.
